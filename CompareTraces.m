%Compare the reference trace (AM_G,r_r) to the automated trace (AM_A, r_A).
function IndexStr = CompareTraces(AM_G, r_G, AM_A, r_A, h_length,h_bp,h_tp)
%AM: Adjacency Matrix (n x n)
%r:  position vector  (n x 3)
%h_length: threshold to find corresponding trace nodes
%h_bp: threshold to find corresponding branch nodes
%h_tp: threshold to find corresponding terminal nodes


%If true, each disjoint tree in A will be
%compared with each disjoint tree in B.
compare_tree_level=false;

%Threshold to remove short branches
h_short_br=15;

%Overall threshold
h=max([h_length;h_bp;h_tp]);

%Compares trees only if the corresponding parts exceed this length 
s=100;

trace=CompareStructure(AM_G,r_G,AM_A,r_A,h,h_length,h_bp,h_tp,h_short_br);

%If there is only one pair of structures to compare
if compare_tree_level
...
for k=1:length(s)
    ...
    for i=1:length(Labels_G)
        ...
        for j=1:length(Labels_A)
            ...
            temp=CompareStructure(AMg, rg, AMa, ra, h,h_length,h_bp,h_tp,h_short_br);
...
%CompareStructure compares traces, trees, or branches depending on input
function IndexStr = CompareStructure(AMg, rg, AMa, ra, h, h_length ,h_bp, h_tp, h_short_br)
